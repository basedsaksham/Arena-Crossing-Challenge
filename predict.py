...
def load_tabular_models():
    global _tabular
    if _tabular is None:
        if os.path.exists("tabular_models.pkl.xz"):
            with lzma.open("tabular_models.pkl.xz", "rb") as f:
                _tabular = pickle.load(f)
            print("Loaded tabular_models.pkl.xz")
        else:
            try:
...
def load_traj_residual_models():
    ...
    if _traj_residual is None:
        if os.path.exists("traj_residual_models.pkl.xz"):
            with lzma.open("traj_residual_models.pkl.xz", "rb") as f:
                _traj_residual = pickle.load(f)
            print("Loaded traj_residual_models.pkl.xz")
        else:
            try:
